"""TCCM popularity branch (PyTorch).

Two modules used at the score-fusion stage:

* :class:`TCCMPopularityEncoder` — content-aware popularity encoder
  driven by per-token bucketed CTR + a reciprocal-power timeliness
  module (paper ``get_popularity_encoder`` recipe).
* :class:`TCCMActivityGater` — per-user gate
  ``Dense(128, tanh) → Dense(64) → Dense(1, sigmoid)``.

The relevance branch (news / user encoder) is reused from PP-Rec — see
:mod:`src.frameworks.pytorch.models.tccm.model`.
"""
