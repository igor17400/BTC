        ...
        *,
        ...
        self.title_intent_W = nnx.Param(
            nnx.initializers.glorot_uniform()(
                rngs.params(), (config.intent_embedding_dim, config.attention_dim)
            )
        )
...
class CROWNUserEncoder(nnx.Module):
    ...
    def __init__(
        self, config: CROWNConfig, news_encoder: CROWNNewsEncoder, *, rngs: nnx.Rngs
    ):
        self.config = config
        ...
        if config.gnn_type == "gat":
            ...
            self.gnn_layers = nnx.List(
                [
                    BipartiteSAGELayer(
                        dim=news_emb_dim, dropout_rate=config.dropout_rate, rngs=rngs
                    )
                    for _ in range(config.graph_num_layers)
                ]
            )
