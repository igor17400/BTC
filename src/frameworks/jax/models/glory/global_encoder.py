"""GLORY global encoders (Flax NNX).

Mirror of the PyTorch ``global_encoder`` module. Hosts the auxiliary
encoders used on the "global" / KG enrichment path:

- :class:`GatedGraphConv` — pure-JAX port of
  ``torch_geometric.nn.GatedGraphConv`` (Li et al. 2016) used to
  propagate features across the precomputed news graph.
- :class:`GRUCell` — JAX/Flax NNX does not provide a standalone GRUCell,
  so we implement one with the same gate equations and default
  initialisation as ``torch.nn.GRUCell``.
...
class GatedGraphConv(nnx.Module):
    ...
    with a single :class:`GRUCell` mixing messages into node states
    (matches GLORY's original configuration which uses the PyG default).
...
class EntityEncoder(nnx.Module):
    ...
    Pipeline: ``emb → dropout → MHA → LN → drop → pool → LN → Dense → LeakyReLU``.
...
class GlobalEntityEncoder(nnx.Module):
    ...
    ``Dense + LeakyReLU``. Output dim = ``head_num * head_dim = news_dim``.
    """
