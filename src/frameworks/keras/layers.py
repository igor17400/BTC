...
class GraphAttentionLayer(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.W_user = self.add_weight(
            name='W_user', shape=(self.num_heads, user_dim, self.head_dim),
            initializer=keras.initializers.GlorotUniform(seed=self.seed), trainable=True
        )
        self.W_news = self.add_weight(
            name='W_news', shape=(self.num_heads, news_dim, self.head_dim),
            initializer=keras.initializers.GlorotUniform(seed=self.seed), trainable=True
        )
        self.a_user_news = self.add_weight(
            name='a_user_news', shape=(self.num_heads, self.head_dim * 2, 1),
            initializer=keras.initializers.GlorotUniform(seed=self.seed), trainable=True
        )
        self.a_news_user = self.add_weight(
            name='a_news_user', shape=(self.num_heads, self.head_dim * 2, 1),
            initializer=keras.initializers.GlorotUniform(seed=self.seed), trainable=True
        )
        if self.use_bias:
            self.b_user = self.add_weight(
                name='b_user', shape=(self.units if self.concat_heads else self.head_dim,),
                initializer=keras.initializers.Zeros(), trainable=True
            )
            self.b_news = self.add_weight(
                name='b_news', shape=(self.units if self.concat_heads else self.head_dim,),
                initializer=keras.initializers.Zeros(), trainable=True
            )
        self.dropout = layers.Dropout(self.dropout_rate, seed=self.seed)
        super().build(input_shape)
