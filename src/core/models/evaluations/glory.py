...
def glory_evaluate(
    *,
    ...
    all_news_emb = adapter.encode_glory_news(
        news_encoder, news_features, batch_size=batch_size,
    )  # (num_news, D)
    ...
    all_news_graph_emb = adapter.encode_glory_global(
        graph_encoder, all_news_emb, graph["edge_index"],
    )  # (num_news, D)
    ...
    for idx in range(num_impressions):
        ...
        scores = adapter.score_glory_impression(
            click_encoder, user_encoder, candidate_encoder, click_predictor,
            clicked_title, clicked_graph, cand_local,
        )  # (C,)
