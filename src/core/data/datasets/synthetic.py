...
class SyntheticDataset:
    ...
    def _build(self):
        ...
        if self.process_entities:
            self._processed_news["entity_indices"] = rng.integers(
                0, self.NUM_ENTITIES, (N, self.max_entities), dtype=np.int32
            )
            ...
            self._processed_news["entity_vocab_size"] = self.NUM_ENTITIES
    ...
    def _make_behaviors(self, n: int) -> dict[str, Any]:
        ...
        if self.process_entities:
            ...
            result["candidate_news_ctr"] = rng.integers(0, 200, (n, K), dtype=np.int32)
            result["candidate_news_recency"] = rng.integers(
                0, 1500, (n, K), dtype=np.int32
            )
        return result
