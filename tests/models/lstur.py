...
def create_dummy_data(batch_size=2, num_users=1000):
    ...
    data = {
        ...
        "single_candidate_tokens": np.random.randint(
            1, 1000, (batch_size, 50), dtype=np.int32
        ),
...
def test_lstur_components(lstur_model, batch_size=2):
    ...
    assert news_output.shape == (
        batch_size,
        expected_dim,
    ), f"Expected shape (batch_size, {expected_dim}), got {news_output.shape}"

    # Test user encoder with 2D user IDs
    print("\n2. Testing user encoder with 2D user IDs...")
    user_output_2d = lstur_model.user_encoder(
        [dummy_data["hist_tokens"], dummy_data["user_ids_2d"]], training=False
    )
    print(f"   ✓ User encoder (2D user IDs) output shape: {user_output_2d.shape}")
    expected_dim = lstur_model.config.gru_unit
    assert user_output_2d.shape == (
        batch_size,
        expected_dim,
    ), f"Expected shape (batch_size, {expected_dim}), got {user_output_2d.shape}"

    # Test user encoder with 1D user IDs (for fast evaluation)
    print("\n3. Testing user encoder with 1D user IDs...")
    user_output_1d = lstur_model.user_encoder(
        [dummy_data["hist_tokens"], dummy_data["user_ids_1d"]], training=False
    )
    print(f"   ✓ User encoder (1D user IDs) output shape: {user_output_1d.shape}")
    assert user_output_1d.shape == (
        batch_size,
        expected_dim,
    ), f"Expected shape (batch_size, {expected_dim}), got {user_output_1d.shape}"
    ...
    assert np.all(single_scores >= 0) and np.all(single_scores <= 1), (
        "Sigmoid outputs should be in [0, 1]"
    )
    print("   ✓ Single candidate scores are valid sigmoid outputs")
    ...
    assert multi_scores.shape == (
        batch_size,
        5,
    ), f"Expected shape (batch_size, 5), got {multi_scores.shape}"
...
def test_lstur_call_method(lstur_model, batch_size=2):
    ...
    assert single_output.shape == (
        batch_size,
        1,
    ), f"Expected shape (batch_size, 1), got {single_output.shape}"

    # Verify sigmoid output
    assert np.all(single_output >= 0) and np.all(single_output <= 1), (
        "Sigmoid outputs should be in [0, 1]"
    )
    print("   ✓ Single candidate outputs are valid sigmoid scores")
    ...
    print("   ✓ Multiple candidate outputs are valid sigmoid scores")
...
def test_lstur_compatibility_models(lstur_model, batch_size=2):
    ...
    assert scoring_output.shape == (
        batch_size,
        1,
    ), f"Expected shape (batch_size, 1), got {scoring_output.shape}"

    # Verify sigmoid output
    assert np.all(scoring_output >= 0) and np.all(scoring_output <= 1), (
        "Sigmoid outputs should be in [0, 1]"
    )
...
def test_lstur_encoder_types(batch_size=2):
    ...
    user_output_ini = lstur_ini.user_encoder(
        [dummy_data["hist_tokens"], dummy_data["user_ids_2d"]], training=False
    )
    print(f"   ✓ 'ini' encoder output shape: {user_output_ini.shape}")
    assert user_output_ini.shape == (
        batch_size,
        300,
    ), f"Expected shape (batch_size, 300), got {user_output_ini.shape}"
    ...
    user_output_con = lstur_con.user_encoder(
        [dummy_data["hist_tokens"], dummy_data["user_ids_2d"]], training=False
    )
    print(f"   ✓ 'con' encoder output shape: {user_output_con.shape}")
    assert user_output_con.shape == (
        batch_size,
        300,
    ), f"Expected shape (batch_size, 300), got {user_output_con.shape}"
