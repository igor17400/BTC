...
class EmbeddingValidator:
    ...
    def test_basic_properties(self) -> dict[str, Any]:
        ...
        results = {
            ...
            "percent_zeros": 100
            * np.sum(self.embedding_matrix == 0)
            / self.embedding_matrix.size,
        }
        return results
    ...
    def test_vector_norms(self) -> dict[str, Any]:
        ...
        if len(non_zero_norms) > 0:
            results["norms_in_reasonable_range"] = (results["min_norm"] > 0.1) and (
                results["max_norm"] < 10
            )
    ...
    def assess_quality(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        if norms["percent_zero_vectors"] > 50:
            warnings.append(
                f"High percentage of zero vectors ({norms['percent_zero_vectors']:.1f}%)"
            )
    ...
    def print_results(self, results: dict[str, Any]):
        ...
        print(
            f"  • Mean ± Std: {basic['mean_value']:.4f} ± {basic['std_deviation']:.4f}"
        )
        ...
        print(
            f"  • Zero vectors: {norms['num_zero_vectors']:,} ({norms['percent_zero_vectors']:.1f}%)"
        )
        ...
        if "error" not in sim:
            print(
                f"  • Range: [{sim['min_similarity']:.4f}, {sim['max_similarity']:.4f}]"
            )
            print(
                f"  • Mean ± Std: {sim['mean_similarity']:.4f} ± {sim['std_similarity']:.4f}"
            )
...
def main():
    ...
    if len(all_results) == 2:
        ...
        if jp_results and en_results:
            ...
            print(
                f"  • Japanese: {jp_basic['vocab_size']:,} words × {jp_basic['embedding_dim']} dims"
            )
            print(
                f"  • English: {en_basic['vocab_size']:,} words × {en_basic['embedding_dim']} dims"
            )
